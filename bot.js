...
const commands = {
	'help': (msg) => {
	  msg.channel.send({embed: {
			...
			{
				name: 'invite',
				value: 'A way to invite the bot.',
				inline: true
			},
			{
				name: 'init',
				value: 'Initalize advertisement channel.',
				inline: true
			},
			{
				name: 'desc',
			...
			{
				name: 'bump',
				value: 'Advertise your guild to everyone.',
				inline: true
			},
			]
		}});
	...
	'init': (msg) => {
		...
		if (channel){
			...
			sendEmbed(msg, msg.guild.id, msg.channel.id, 'Channel sucessfully synced.');
		} else {
	...
	'desc': (msg) => {
		...
		if (string === undefined || string === ''){
			return sendEmbed(msg, msg.guild.id, msg.channel.id, 'Please specifty a description.');
		}
		if (string.length > 255){
			return sendEmbed(msg, msg.guild.id, msg.channel.id, 'Description can not be anymore than 255 characters long.');
		}
		if (string.length < 30){
		...
		sendEmbed(msg, msg.guild.id, msg.channel.id, 'Description updated sucessfully.');
	},
	'preview': (msg) => {
		if (!msg.member.hasPermission('ADMINISTRATOR')){
			return sendEmbed(msg, msg.guild.id, msg.channel.id, 'You need to have the administrator permission to do this.');
